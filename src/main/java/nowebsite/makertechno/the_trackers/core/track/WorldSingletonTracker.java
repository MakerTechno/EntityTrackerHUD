...
public class WorldSingletonTracker {
     ...
     * @return 返回注册成功的指针。
     */
    public SimpleComponent addDynamicComponent(String modID, String identifyName, ComponentBuilder.BuilderResult result, UUID target, boolean isVisible) {
        synchronized (questSortedEntityPointers) {
            ...
            ControllableTrackerState state = new ControllableTrackerState(identifyName, BuilderResultComposer.compose(result), isVisible);
     ...
     * @return 返回注册成功的指针。
     */
    public SimpleComponent addDynamicTickLimitedComponent(String modID, String identifyName, ComponentBuilder.BuilderResult result, UUID target, int tickForTime, boolean isVisible) {
        synchronized (questSortedEntityPointers) {
            if (questSortedEntityPointers.containsKey(target)) throw new IllegalArgumentException("UUID named " + target + " still on tracking!");
     ...
     */
    public SimpleComponent addDynamicMultiComponent(String modID, String identifyName, ComponentBuilder.BuilderResult[] results, UUID target, boolean isVisible) {
        synchronized (questSortedEntityPointers) {
            if (questSortedEntityPointers.containsKey(target)) throw new IllegalArgumentException("UUID named " + target + " still on tracking!");
            ...
            ControllableTrackerStateEx state = new ControllableTrackerStateEx(identifyName, components, isVisible);
            entityPointers.computeIfAbsent(modID, s -> new ArrayList<>()).add(state);
            questSortedEntityPointers.put(target, state);
            return state;
        }
     ...
     * @return 返回注册成功的指针。
     */
    public SimpleComponent attendMultiComponent(String modID, String identifyName, ComponentBuilder.BuilderResult[] results, UUID target) {
        ...
        synchronized (questSortedEntityPointers) {
            ...
            ControllableTrackerStateEx stateEx = new ControllableTrackerStateEx(identifyName, components, trackerState.isVisible());
            trackerState.close();
    ...
    public void trigger(Map<UUID, Entity> cached) {
        ...
        if (!questSortedEntityPointers.isEmpty()) {
            ...
            cached.forEach((uuid, entity) -> questSortedEntityPointers.get(uuid).setPos(entity.position()));
        }
        ...
        if (!staticPointers.isEmpty()) {
            ...
            staticPointers.forEach((s, states) -> states.forEach(state -> state.setPos(player.position())));
        }
    }
